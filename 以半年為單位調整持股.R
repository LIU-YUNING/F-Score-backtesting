...
#FR<-read.table("new_FR.txt",header=T,sep=",")
colnames(FR)<-c("code","name","yearmonth","cashflow","profit","debt","current_ratio","share","gross","ROA","asset_turnover")
...
#=年報=====================================================================
for(report in c(6,12))
{
  fscore<-FR %>%filter(yearmonth%%100==report)%>%
  ...
  FR_year<-unique(fscore$yearmonth)
  ...
  for(year in FR_year)
  {
    #if(year==201512|year==201506){next}#2015只是用來得出2016年財報的f-score
    if(year%%100==12)
    {
      fr_year=floor(year/100)
      sp_year=fr_year+1#購買年分年報+1
      
      sp_start_date=sp_year*10000+331#從年報公布約(3/31)之後開始
      sp_end_date=(sp_year)*10000+814#到半年報出來為止(8/14)
      thisFR<-fscore %>%
    ...
    {
      fr_year=floor(year/100)
      sp_year=fr_year#購買年分為財報年分
      
      sp_start_date=sp_year*10000+814#從年報公布約(3/31)之後開始
      sp_end_date=(sp_year+1)*10000+331#到半年報出來為止(8/14)
      thisFR<-fscore %>%
        filter(yearmonth==year)
      thisSP3<-SP %>%
        filter(date>=sp_start_date,
      ...
      sp_start_date=sp_year*10000+814-5#-5：策略一要算5MA
